...
import com.crystal.aplayer.module_base.tools.preferences.PreferenceTools.getString
import java.util.*
...
object GlobalUtil {
        ...
        if (deviceSerial == null) {
            ...
            if ("google" != appChannel || "samsung" != appChannel) {
                try {
                    ...
                    LogUtil.w(TAG, "get android_id with error")
                }
            ...
            var uuid = getString(BaseApplication.getContext(), "uuid", "")
            if (!TextUtils.isEmpty(uuid)) {
                deviceSerial = uuid
                return deviceSerial.toString()
            }
            uuid = UUID.randomUUID().toString().replace("-", "").toUpperCase(Locale.getDefault())
            PreferenceTools.putString(BaseApplication.getContext(),"uuid", uuid)
            deviceSerial = uuid
            return deviceSerial.toString()
        } else {
            return deviceSerial.toString()
    ...
    fun getApplicationMetaData(key: String): String? {
        ...
        try {
            ...
            LogUtil.w(TAG, e.message.toString())
        }
